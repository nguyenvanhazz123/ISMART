...
function get_post_cat_by_id($id){
    $result = db_fetch_row("SELECT * FROM `tbl_post_cat` WHERE `id_post_cat` = $id ");
    return $result;
}
...
function get_post_cat_page($start = 1, $num_per_page = 10, $where = ""){
    if(!empty($where)){
        $where = " WHERE {$where}"; 
    }
    $list_post = db_fetch_array("SELECT * FROM `tbl_post_cat` {$where} LIMIT {$start}, {$num_per_page}");
    return $list_post;
}
...
function get_post_cat_name_by_id($id){
    $result = db_fetch_row("SELECT * FROM `tbl_post_cat` WHERE `id_post_cat` = $id ");
    return $result;
}
function get_num_post_pending(){
    $result = db_num_rows("SELECT * FROM `tbl_post` WHERE `status` = 1");
    return $result;
}
function get_num_post_posted(){
    $result = db_num_rows("SELECT * FROM `tbl_post` WHERE `status` = 2");
    return $result;
}
