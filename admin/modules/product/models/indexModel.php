...
function get_list_brand(){
    $result = db_fetch_array("SELECT * FROM `tbl_product_brand`");
    return $result;
}
...
function get_id_max_product_cat(){
    $result = db_query("SELECT MAX(`id`) FROM `tbl_product_cat`");
    if ($result) {
...
function get_num_product_pending(){
    $result = db_num_rows("SELECT * FROM `tbl_product` WHERE `status` = 1");
    return$result;
}
function get_num_product_posted(){
    $result = db_num_rows("SELECT * FROM `tbl_product` WHERE `status` = 2");
    return$result;
}
    ...
    if(isset($_FILES['file'])){
        ...
        //Kiểm tra và thay đường dẫn cũ thành đường dẫn mới             
            if(move_uploaded_file($_FILES['file']['tmp_name'], $upload_file)){                   
                $thumb = "public/uploads/"."{$_FILES['file']['name']}";                                                        
            }else{
